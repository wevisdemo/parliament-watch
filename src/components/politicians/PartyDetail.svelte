<script lang="ts">
	...
	interface Props {
		party: string;
		memberships: (Pick<Membership, 'start_date' | 'end_date'> & {
			posts: Pick<Post, 'role'>[];
		})[];
	}

	let { party, memberships }: Props = $props();

	// NOTE: assume ว่า data จะ sort มาแล้ว + มีข้อมูลอย่างน้อย 1
	let partyFrom = $derived(memberships.at(-1)?.start_date);
	let partyTo = $derived(memberships.at(0)?.end_date);
</script>
